...
-----------------------------------------------*/
ENT.Model = {"models/vj_cthulhu/humanoid_ghoul.mdl"} -- The game will pick a random model from the table when the SNPC is spawned | Add as many as you want
ENT.StartHealth = 115
...
ENT.SoundTbl_Idle = {}
ENT.SoundTbl_Alert = {"vj_cthulhu/ghoul/alert1.wav","vj_cthulhu/ghoul/alert2.wav","vj_cthulhu/ghoul/alert3.wav"}
ENT.SoundTbl_BeforeMeleeAttack = {"vj_cthulhu/ghoul/attack1.wav","vj_cthulhu/ghoul/attack2.wav"}
...
ENT.SoundTbl_Pain = {}
ENT.SoundTbl_Death = {"vj_cthulhu/ghoul/die1.wav","vj_cthulhu/ghoul/die2.wav","vj_cthulhu/ghoul/zombie_die1.wav"}
...
function ENT:MultipleMeleeAttacks()
	if math.random(1, 2) == 1 then
		self.AnimTbl_MeleeAttack = {"attack1","attack2","attack5"}
...
function ENT:CustomOnDeath_AfterCorpseSpawned(dmginfo, hitgroup, corpseEnt)
	...
	timer.Simple(math.random(2,4), function()
		if IsValid(corpseEnt) then
			local Evolution = ents.Create("npc_vj_cthulhu_humanoid_ghoul")
			...
			if Evolution.EvolveNow == true then
				...
				VJ_EmitSound(Evolution, {"vj_cthulhu/ghoul/ropp.wav"}, 70, math.random(100, 80))
	               local ReviveAnim = VJ_PICK({"wallseen","lockeraction","bedappear"})
...
function ENT:DoKilledEnemy(victim,attacker,inflictor)
         if victim.IsVJBaseSNPC_Human or victim.IsVJBaseSNPC_Creature or victim:IsPlayer() then
            npc = ents.Create("npc_vj_cthulhu_humanoid_ghoul")
