...
-----------------------------------------------*/
ENT.Model = {"models/vj_cthulhu/cthonian.mdl"} -- The game will pick a random model from the table when the SNPC is spawned | Add as many as you want
...
ENT.NextRangeAttackTime = 29 -- How much time until it can use a range attack?
...
function ENT:CustomOnAcceptInput(key, activator, caller, data)
	...
	elseif key == "burst" then
        ...
        self:CreateGibEntity("obj_vj_gib", "models/vj_cthulhu/bigrock.mdl", { BloodDecal = "", Pos = self:LocalToWorld(Vector(-350, -15, 35)),CollideSound={}})
        self:CreateGibEntity("obj_vj_gib", "models/vj_cthulhu/bigrock.mdl", { BloodDecal = "", Pos = self:LocalToWorld(Vector(-350, -15, 35)),CollideSound={}})
	util.Decal("VJ_CTHULHU_Scorch", self:GetPos() + self:GetRight()*20 + self:GetForward()*-350, self:GetPos() + self:GetRight()*20 + self:GetForward()*-350 + self:GetUp()*-100, self)
...
function ENT:CustomOnInitialize()
	...
	timer.Simple(5.38,function() if IsValid(self) then self:SetPos(self:GetPos()+ self:GetForward()*-190) end end)
	timer.Simple(5.38,function() if IsValid(self) then self.MovementType = VJ_MOVETYPE_GROUND end end)
end
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
