/* Note: All credits go to Cpt. Hazama and darkborn. I take no credit for this. */
...
ENT.Monster = {
	{class="npc_vj_cthulhu_cultist_knife",chance=1},
	{class="npc_vj_cthulhu_cultist_revolver",chance=1},
	{class="npc_vj_cthulhu_cultist_shotgun",chance=1},
	{class="npc_vj_cthulhu_snake",chance=1},
	{class="npc_vj_cthulhu_eyeegg",chance=1},
	{class="npc_vj_cthulhu_zombie",chance=1},
	{class="npc_vj_cthulhu_humanoid_ghoul",chance=1},
	{class="npc_vj_cthulhu_greatrace",chance=1},
	{class="npc_vj_cthulhu_nightgaunt",chance=1},
	{class="npc_vj_cthulhu_yodan",chance=2},	{class="npc_vj_cthulhu_serpentman",chance=2},	{class="npc_vj_cthulhu_priest",chance=2},	{class="npc_vj_cthulhu_formless_spawn",chance=2},
	{class="npc_vj_cthulhu_humanoid_formless_spawn",chance=2},
	{class="npc_vj_cthulhu_deepone",chance=2},
	{class="npc_vj_cthulhu_ghoul",chance=2},
	{class="npc_vj_cthulhu_fatheadlesszombie",chance=2},
	{class="npc_vj_cthulhu_huntinghorror",chance=3},
	{class="npc_vj_cthulhu_shambler",chance=3},	{class="npc_vj_cthulhu_greatrace_zikshadows",chance=4},
	{class="npc_vj_cthulhu_yodan_zikshadows",chance=4},
}

ENT.BossMonster = {
	{class="npc_vj_cthulhu_cthonian",max=1},
	{class="npc_vj_cthulhu_cthulhu",max=1},
	{class="npc_vj_cthulhu_dagon",max=1},
	{class="npc_vj_cthulhu_eyetentacle",max=1},
	{class="npc_vj_cthulhu_hideousmeatmass",max=1},	
}
---------------------------------------------------------------------------------------------------------------------------------------------
function ENT:Initialize()
	local i = 0
	for k, v in ipairs(ents.GetAll()) do
		if v:GetClass() == "sent_vj_cthulhu_mapspawner_remod" then
			i = i + 1
			if i > 1 then PrintMessage(HUD_PRINTTALK, "Dude is hard for you to understand that you can't spawn a lot of map spawners?.") self.SkipOnRemove = true self:Remove() return end
		end
	end
	...
	for _,pos in pairs(VJ_CTHULHU_NODEPOS) do
	...
	self.IsActivated = tobool(GetConVarNumber("VJ_CTHULHU_MapSpawner_Enabled"))
	self.HR_SpawnDistance = GetConVarNumber("VJ_CTHULHU_MapSpawner_SpawnMax")
	self.HR_SpawnDistanceClose = GetConVarNumber("VJ_CTHULHU_MapSpawner_SpawnMin")
	self.HR_HordeChance = GetConVarNumber("VJ_CTHULHU_MapSpawner_HordeChance")
	self.HR_HordeCooldownMin = GetConVarNumber("VJ_CTHULHU_MapSpawner_HordeCooldownMin")
	self.HR_HordeCooldownMax = GetConVarNumber("VJ_CTHULHU_MapSpawner_HordeCooldownMax")
	self.HR_MaxMonster = GetConVarNumber("VJ_CTHULHU_MapSpawner_MaxMon")
	self.HR_MaxHordeSpawn = GetConVarNumber("VJ_CTHULHU_MapSpawner_HordeCount")
	...
	self.NextHordeSpawnTime = CurTime() +math.Rand(self.HR_HordeCooldownMin,self.HR_HordeCooldownMax)
	...
	self.CanSpawnBossMonster = false --GetConVarNumber("VJ_CTHULHU_MapSpawner_Boss")
	
	for _,v in ipairs(player.GetAll()) do
		   //v:ChatPrint("what i am even doing in here")
		if GetConVarNumber("VJ_CTHULHU_MapSpawner_Music") == 1 then
		   self.COFR_Music = VJ_CreateSound(v,"vj_cthulhu/soundtrack/heavepiercedyourmother.mp3",GetConVarNumber("VJ_HR_MapSpawner_MusicVolume"), 100)	
		end
	end	
end
---------------------------------------------------------------------------------------------------------------------------------------------
function ENT:CheckVisibility(pos,ent,mdl)
	...
	check:SetModel(mdl or "models/vj_cthulhu/eyeegg.mdl")
	check:SetPos(pos)
...
function ENT:FindCenterNavPoint(ent)
	for _,v in RandomPairs(self.navAreas) do
		...
		if dist <= self.HR_SpawnDistance && dist >= self.HR_SpawnDistanceClose && !self:CheckVisibility(testPos,ent) then
			return testPos
		end
	end
	return false
...
function ENT:GetClosestNavPosition(ent,getHidden)
	...
	for i,v in pairs(self.navAreas) do
		...
		if dist < closestDist && (dist <= self.HR_SpawnDistance && dist >= self.HR_SpawnDistanceClose && !self:CheckVisibility(testPos,ent)) then
			closestDist = dist
			pos = testPos
		end
	end
...
function ENT:FindEnemy()
	...
	for _,v in pairs(ents.GetAll()) do
		if (v:IsPlayer() || v:IsNPC()) && v:Health() > 0 && !v:IsFlagSet(65536) && (v.VJ_NPC_Class && !VJ_HasValue(v.VJ_NPC_Class,"CLASS_CTHULHU") or true) then
			table_insert(tbl,v)
		end
	end
	return tbl
...
function ENT:CheckEnemyDistance(ent,remove)
	...
	if closestDist >= GetConVarNumber("VJ_CTHULHU_MapSpawner_SpawnMax") +1000 && !visible && !remove then
		SafeRemoveEntity(ent)
	end
end
---------------------------------------------------------------------------------------------------------------------------------------------
function ENT:Think()
	self.IsActivated = GetConVar("VJ_CTHULHU_MapSpawner_Enabled")
	if self.IsActivated then 
	    ...
	    self.HR_MaxMonster = GetConVarNumber("VJ_CTHULHU_MapSpawner_MaxMon")
	    self.HR_MaxHordeSpawn = GetConVarNumber("VJ_CTHULHU_MapSpawner_HordeCount")
		self.AI_RefreshTime = GetConVarNumber("VJ_CTHULHU_MapSpawner_RefreshRate") 
		...
		if CurTime() > self.NextMonsterSpawnTime then
			if #self.tbl_SpawnedNPCs >= self.HR_MaxMonster -self.HR_MaxHordeSpawn then return end -- Makes sure that we can at least spawn a mob when it's time
			self:SpawnMonster(self:PickMonster(self.Monster),self:FindSpawnPosition(false))
			self.NextMonsterSpawnTime = CurTime() +math.Rand(GetConVarNumber("VJ_CTHULHU_MapSpawner_DelayMin"),GetConVarNumber("VJ_CTHULHU_MapSpawner_DelayMax"))
		end

		if GetConVarNumber("VJ_CTHULHU_MapSpawner_Boss") == 1 then
		...
		-- Spawns Hordes
		if CurTime() > self.NextHordeSpawnTime && math.random(1,self.HR_HordeChance) == 1 then
			for i = 1,self.HR_MaxHordeSpawn do
...
function ENT:SpawnMonster(ent,pos,isMob)
	...
	if #self.tbl_SpawnedNPCs >= self.HR_MaxMonster then return end
