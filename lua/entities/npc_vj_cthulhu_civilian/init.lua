...
-----------------------------------------------*/
ENT.Model = {"models/vj_cthulhu/civilian.mdl"} -- The game will pick a random model from the table when the SNPC is spawned | Add as many as you want
ENT.StartHealth = 80
...
function ENT:SCI_CustomOnInitialize()
	local randrace = math.random(1,6)
	...
	if randrace == 4 then self:SetBodygroup(0,3) end
	if randrace == 5 then self:SetBodygroup(0,4) end
	if randrace == 6 then self:SetBodygroup(0,5) end

	self.SoundTbl_Idle = {"vj_cthulhu/civilian/civ_idle1.wav","vj_cthulhu/civilian/civ_idle2.wav","vj_cthulhu/civilian/civ_idle3.wav"}
	self.SoundTbl_OnPlayerSight = {"vj_cthulhu/civilian/civ_phello.wav","vj_cthulhu/civilian/civ_hello.wav"}
...
function ENT:CustomOnDeath_AfterCorpseSpawned(dmginfo, hitgroup, corpseEnt)
	if not IsValid(corpseEnt) then return end

	-- Apply blood effect at corpse location
	local effectBlood = EffectData()
	effectBlood:SetOrigin(corpseEnt:GetPos() + corpseEnt:OBBCenter())
	effectBlood:SetColor(VJ_Color2Byte(Color(130,19,10)))
	effectBlood:SetScale(60)
	util.Effect("VJ_Blood1", effectBlood)

	local bloodspray = EffectData()
	bloodspray:SetOrigin(corpseEnt:GetPos())
	...
	-- Spawn gibs around the corpse
	for _, model in ipairs(gibsgreen) do
		local gib = ents.Create("obj_vj_gib")
		if IsValid(gib) then
			gib:SetModel(model)
			gib:SetPos(corpseEnt:GetPos() + VectorRand(-10, 10) + Vector(0, 0, 30))
			gib:SetAngles(AngleRand())
			gib:Spawn()
			gib:Activate()
			gib.BloodDecal = "VJ_HLR_Blood_Red"
		end
	end
end
