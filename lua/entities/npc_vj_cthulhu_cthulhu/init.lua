...
-----------------------------------------------*/
ENT.Model = {"models/vj_cthulhu/cthulhu.mdl"} -- The game will pick a random model from the table when the SNPC is spawned | Add as many as you want
...
ENT.DeathAnimationTime = 3.1 -- Time until the SNPC spawns its corpse and gets removed
...
ENT.SoundTbl_FootStep = {"vj_cthulhu/garg/gar_step1.wav","vj_cthulhu/garg/gar_step2.wav"}
ENT.SoundTbl_Breath = {"vj_cthulhu/garg/gar_breathe1.wav","vj_cthulhu/garg/gar_breathe2.wav","vj_cthulhu/garg/gar_breathe3.wav"}
ENT.SoundTbl_Alert = {"vj_cthulhu/garg/gar_alert1.wav","vj_cthulhu/garg/gar_alert2.wav","vj_cthulhu/garg/gar_alert3.wav"}
ENT.SoundTbl_BeforeMeleeAttack = {"vj_cthulhu/garg/gar_attack1.wav","vj_cthulhu/garg/gar_attack2.wav","vj_cthulhu/garg/gar_attack3.wav"}
...
ENT.MeleeAttackExtraSoundLevel = 100
...
function ENT:CustomOnAcceptInput(key, activator, caller, data)
	...
	if key == "melee" then
        ...
        self:CreateGibEntity("obj_vj_gib", "models/vj_cthulhu/bigrock.mdl", { BloodDecal = "", Pos = self:LocalToWorld(Vector(250, -5, 0)),CollideSound={}})
        self:CreateGibEntity("obj_vj_gib", "models/vj_cthulhu/bigrock.mdl", { BloodDecal = "", Pos = self:LocalToWorld(Vector(250, -5, 0)),CollideSound={}})
        self:CreateGibEntity("obj_vj_gib", "models/vj_cthulhu/bigrock.mdl", { BloodDecal = "", Pos = self:LocalToWorld(Vector(250, -5, 0)),CollideSound={}})
		VJ_EmitSound(self, {"vj_cthulhu/garg/gar_stomp1.wav"}, 90)
	util.Decal("VJ_CTHULHU_Scorch", self:GetPos() + self:GetRight()*-20 + self:GetForward()*250, self:GetPos() + self:GetRight()*-20 + self:GetForward()*250 + self:GetUp()*-100, self)
	end
	if key == "laser" then
...
function ENT:MultipleMeleeAttacks()
	...
	if r == 1 then
		...
		self.Garg_MeleeLargeKnockback = false
                self.SoundTbl_MeleeAttackExtra = {"vj_cthulhu/garg/gar_stomp1.wav"}
	elseif r == 2 then
		self.AnimTbl_MeleeAttack = {"vjseq_attack"}
...
function ENT:MultipleRangeAttacks()
	...
	elseif self.Garg_NextStompAttackT < CurTime() && self.Garg_Type != 1 then -- Laser stomp attack
		...
		self.SoundTbl_RangeAttack = {"vj_cthulhu/garg/gar_stomp1.wav"}
	else
		self.Garg_AttackType = -1
		self.Garg_AbleToFlame = false
	end
...
local Buglin = ents.Create("npc_vj_cthulhu_cthulhu_2")
Buglin:SetPos(self:GetPos())
Buglin:SetAngles(self:GetAngles())
Buglin:Spawn()
Buglin:Activate()
	for _,v in ipairs(player.GetHumans()) do
		v:ScreenFade(SCREENFADE.IN, colorWhite, 2, 0)
	end
end