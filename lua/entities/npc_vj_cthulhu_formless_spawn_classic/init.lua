...
ENT.Model = {"models/vj_cthulhu/formlessspawn_classic.mdl"} -- The game will pick a random model from the table when the SNPC is spawned | Add as many as you want
ENT.StartHealth = 150
...
ENT.AnimTbl_LeapAttack = {"rattack"} -- Melee Attack Animations
...
function ENT:CustomOnInitialize()
	timer.Simple(0.0,function() if IsValid(self) then self:VJ_ACT_PLAYACTIVITY("spawn", true, false, false) end end)
end
---------------------------------------------------------------------------------------------------------------------------------------------
function ENT:CustomOnDeath_AfterCorpseSpawned(dmginfo, hitgroup, corpseEnt)
	...
	timer.Simple(math.random(5,12), function()
		if IsValid(corpseEnt) then
			local Evolution = ents.Create("npc_vj_cthulhu_formless_spawn_classic")
